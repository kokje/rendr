...
import rendr.{Business, GeoHash}

case class Rating (user_id : Int, rating : Int)
case class Venue  (venue_id: Int, lat: Double, long: Double)

object FoursquareProject {
	
	def main(args : Array[String]) {
		val ratingsFile = "hdfs://ec2-54-193-103-208.us-west-1.compute.amazonaws.com:9000/foursquare/ratings_full.dat" // Dump of foursquare reviews
                val venuesFile = "hdfs://ec2-54-193-103-208.us-west-1.compute.amazonaws.com:9000/foursquare/venues_cleaned.dat" // Dump of foursquare business metadata
		val businessesFile = "hdfs://ec2-54-193-103-208.us-west-1.compute.amazonaws.com:9000/yelp/businesses_full.json" // Dump of yelp business metadata
                
		//val sc = new SparkContext("local", "FoursquareDataSet", "/usr/local/spark",List("target/scala-2.10/foursquareproject_2.10-1.0.jar"))
		
		val conf = new SparkConf().setAppName("FoursquareProject").set("spark.executor.memory", "2g")//.set("spark.eventLog.enabled", "true").set("spark.eventLog.dir", "/mnt/data/sparklog")
		...
		// Todo: filter out rating < 3
		// RDD[VenueID, (Restaurant, Rating)] -> RestaurantID | UserID | Name | City | State
		restaurantsYelpFoursquare.join(ratings).map(line => (line._2._1.business_id, line._2._2.user_id, line._2._1.name, line._2._1.city, line._2._1.state)).collect().foreach(println)	
	}
}
